...
import { deleteMatchDataById } from 'apiSdk/match-data';
  ...
  const { data, error, isLoading, mutate } = useSWR<OrganizationInterface>(
    ...
    () =>
      getOrganizationById(id, {
        relations: ['user', 'match_data', 'prediction', 'team_member'],
      }),
  );

  const match_dataHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await deleteMatchDataById(id);
      await mutate();
    } catch (error) {
              ...
              {hasAccess('match_data', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/match-data/create?organization_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.match_data?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/match-data/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.data}</Td>
                            <Td>
                              {hasAccess('match_data', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/match-data/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('match_data', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    match_dataHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
              ...
              {hasAccess('prediction', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/predictions/create?organization_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
