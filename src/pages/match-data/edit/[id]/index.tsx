...
import { getMatchDataById, updateMatchDataById } from 'apiSdk/match-data';
...
function MatchDataEditPage() {
  ...
  const handleSubmit = async (values: MatchDataInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateMatchDataById(id, values);
      mutate(updated);
      resetForm();
  ...
  const formik = useFormik<MatchDataInterface>({
    initialValues: data,
    validationSchema: matchDataValidationSchema,
