...
import { getBettorPredictionById, updateBettorPredictionById } from 'apiSdk/bettor-predictions';
...
function BettorPredictionEditPage() {
  ...
  const handleSubmit = async (values: BettorPredictionInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateBettorPredictionById(id, values);
      mutate(updated);
      resetForm();
