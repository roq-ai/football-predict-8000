...
import { getBettorById } from 'apiSdk/bettors';
import { Error } from 'components/error';
import { BettorInterface } from 'interfaces/bettor';
import useSWR from 'swr';
import { useRouter } from 'next/router';
import { AccessOperationEnum, AccessServiceEnum, useAuthorizationApi, withAuthorization } from '@roq/nextjs';
import { deleteBettorPredictionById } from 'apiSdk/bettor-predictions';
import { deleteFeedbackById } from 'apiSdk/feedbacks';

function BettorViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<BettorInterface>(
    ...
    () =>
      getBettorById(id, {
        relations: ['user', 'bettor_prediction', 'feedback'],
      }),
  );

  const bettor_predictionHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
    ...
    <AppLayout>
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          ...
          {hasAccess('bettor', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/bettors/edit/${data?.id}`} passHref legacyBehavior>
              <Button
              ...
              {hasAccess('bettor_prediction', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/bettor-predictions/create?bettor_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.bettor_prediction?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/bettor-predictions/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>
                              {hasAccess(
                                ...
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/bettor-predictions/edit/${record.id}`}
                                >
                              ...
                              {hasAccess(
                                ...
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    bettor_predictionHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
              ...
              {hasAccess('feedback', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/feedbacks/create?bettor_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
